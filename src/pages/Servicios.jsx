...
function Servicios() {
  ...
  const cargarDatosIniciales = async () => {
    ...
    const dataLugares = await fetchSafe(supabase.from('lugarejecucion').select('*').eq('activo', true))
  ...
  // Novedad: Seleccionar / Deseleccionar Todo
  const handleSelectAll = (todasLasOpciones, filtroActual, setFiltro) => {
    if (filtroActual.length === todasLasOpciones.length) {
      setFiltro([]) // Deseleccionar todo
    } else {
      setFiltro([...todasLasOpciones]) // Seleccionar todo
    }
  }
  ...
  datosProcesados.sort((a, b) => {
    const getVal = (obj, key) => key === 'lugar' ? obj.lugarejecucion?.lugarejecucion || '' : key === 'sistema' ? obj.sistema?.sistema || obj.tipoinfraestructura?.tipoinfraestructura || '' : obj[key]
    let valA = getVal(a, sortConfig.key) || ''; let valB = getVal(b, sortConfig.key) || '';
  ...
  // Listas de opciones completas para los filtros
  ...
  // Novedad: Componente Reutilizable de Paginación
  const controlesPaginacion = (
    <div style={{ padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#F8FAFC' }}>
      ...
      <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
        <select value={registrosPorPagina} onChange={(e) => { setRegistrosPorPagina(Number(e.target.value)); setPaginaActual(1); }} style={{ padding: '4px 8px', borderRadius: '6px', border: `1px solid ${theme.border}`, fontSize: '12px' }}>
      ...
      <div style={{ maxWidth: '1600px', width: '95%', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
              <input type="text" placeholder="🔍 Buscar servicio, OC..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} style={{ padding: '8px 12px', width: '280px', borderRadius: '6px', border: `1px solid ${theme.border}` }} />
              
              <div style={{ position: 'relative' }}>
                <button onClick={(e) => { e.stopPropagation(); setMostrarMenuEstado(!mostrarMenuEstado); }} style={{ padding: '8px 12px', borderRadius: '6px', border: `1px solid ${theme.border}`, backgroundColor: filtroEstado.length > 0 ? '#EFF6FF' : theme.bgCard, color: filtroEstado.length > 0 ? theme.primary : theme.textMain, cursor: 'pointer' }}>Estados {filtroEstado.length > 0 && `(${filtroEstado.length})`} ▼</button>
                {mostrarMenuEstado && (
                  <div onClick={(e) => e.stopPropagation()} style={{ position: 'absolute', top: '100%', left: 0, backgroundColor: 'white', border: `1px solid ${theme.border}`, borderRadius: '8px', padding: '12px', zIndex: 50, width: '250px', maxHeight: '300px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={(e) => { e.stopPropagation(); setMostrarMenuPrioridad(!mostrarMenuPrioridad); }} style={{ padding: '8px 12px', borderRadius: '6px', border: `1px solid ${theme.border}`, backgroundColor: filtroPrioridad.length > 0 ? '#EFF6FF' : theme.bgCard, color: filtroPrioridad.length > 0 ? theme.primary : theme.textMain, cursor: 'pointer' }}>Prioridad {filtroPrioridad.length > 0 && `(${filtroPrioridad.length})`} ▼</button>
                {mostrarMenuPrioridad && (
                  <div onClick={(e) => e.stopPropagation()} style={{ position: 'absolute', top: '100%', left: 0, backgroundColor: 'white', border: `1px solid ${theme.border}`, borderRadius: '8px', padding: '12px', zIndex: 50, width: '180px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 'bold', paddingBottom: '6px', borderBottom: `1px solid ${theme.border}` }}>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={(e) => { e.stopPropagation(); setMostrarMenuLugar(!mostrarMenuLugar); }} style={{ padding: '8px 12px', borderRadius: '6px', border: `1px solid ${theme.border}`, backgroundColor: filtroLugar.length > 0 ? '#EFF6FF' : theme.bgCard, color: filtroLugar.length > 0 ? theme.primary : theme.textMain, cursor: 'pointer' }}>Lugares {filtroLugar.length > 0 && `(${filtroLugar.length})`} ▼</button>
                {mostrarMenuLugar && (
                  <div onClick={(e) => e.stopPropagation()} style={{ position: 'absolute', top: '100%', left: 0, backgroundColor: 'white', border: `1px solid ${theme.border}`, borderRadius: '8px', padding: '12px', zIndex: 50, width: '250px', maxHeight: '300px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 'bold', paddingBottom: '6px', borderBottom: `1px solid ${theme.border}` }}>
        ...
        <div style={{ backgroundColor: theme.bgCard, borderRadius: '12px', border: `1px solid ${theme.border}`, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${theme.border}` }}>
            ...
            <div style={{ position: 'relative', paddingRight: '16px', borderLeft: `1px solid ${theme.border}`, height: '100%', display: 'flex', alignItems: 'center', paddingLeft: '16px', backgroundColor: '#F8FAFC' }}>
              ...
              {mostrarMenuColumnas && (
                <div onClick={(e) => e.stopPropagation()} style={{ position: 'absolute', right: 16, top: '100%', backgroundColor: 'white', border: `1px solid ${theme.border}`, borderRadius: '8px', padding: '12px', zIndex: 50, width: '200px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                  <tr>
                    ...
                    {columnas.idPri && <th style={thStyle} onClick={() => handleSort('idservicio')}>ID/Pri</th>}
                    {columnas.fechaCreacion && <th style={thStyle} onClick={() => handleSort('fechasolicitud')}>F. Creación</th>}
                    {columnas.servicio && <th style={thStyle} onClick={() => handleSort('servicio')}>Servicio</th>}
                    {columnas.lugar && <th style={thStyle} onClick={() => handleSort('lugar')}>Lugar</th>}
                    {columnas.responsable && <th style={thStyle} onClick={() => handleSort('responsable')}>Responsable</th>}
                    {columnas.docs && <th style={thStyle} onClick={() => handleSort('num_requerimiento')}>Req / OC</th>}
                    {columnas.fechasNuevas && <th style={thStyle} onClick={() => handleSort('solped')}>Solped</th>}
                    {columnas.progreso && <th style={thStyle} onClick={() => handleSort('progreso')}>Progreso</th>}
                    {columnas.estado && <th style={{...thStyle, textAlign: 'center'}} onClick={() => handleSort('estado')}>Estado</th>}
                  ...
                  {datosPaginados.map(srv => {
                    ...
                    return (
                      <React.Fragment key={srv.idservicio}>
                        ...
                        {expandido && (
                          <tr style={{ backgroundColor: theme.bgSubTable }}><td colSpan="11" style={{ padding: '16px' }}>Cotizaciones expandidas (Simplificado)</td></tr>
                        )}
                      </React.Fragment>
                    )
                  })}
        ...
        <ServicioModal 
          ...
          onAbrirNuevoLugar={() => setMostrarModalLugar(true)}
        />
